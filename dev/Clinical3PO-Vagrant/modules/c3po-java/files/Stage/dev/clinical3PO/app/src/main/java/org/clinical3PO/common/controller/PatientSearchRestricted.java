...
@Controller
@RequestMapping("/PatientSearchRestricted")
public class PatientSearchRestricted {
	...
	public String getPatient(@Valid @ModelAttribute("patientSearchForm") PatientSearchForm patientForm, BindingResult result, Model model) {
		...
		jobSearchDetails.setSearchOn(SearchOn.PATIENTIDRESTRICTED.getSearchOn());
		jobSearchDetails.setSearchType("PatientIdRestricted");
		jobSearchDetails.setSearchParameters(jobSearch.getSearchParameters().get(0).getValue());
		jobSearchDetails.setScriptType(SearchScript.PATIENTIDRESTRICTED.getSearchScript());
