...
//---------------------C3PO Start ---
    ...
    {
        QBitArray map(256, false);
        fillBitArray(map, "ABCDEFGHIJKLMNOPQRSTUVWYZ", Qt::CaseInsensitive);

        const DNAAlphabet* a = new DNAAlphabet(BaseDNAAlphabetIds::C3PO_ALPHABET(), tr("C3PO Type"), C3POAlphabet_TEST, map, Qt::CaseInsensitive, 'X');
        registerAlphabet(a);
    }
//---------------------C3PO End ---
