...
@Controller
@RequestMapping("/ObservationSearch")
public class ObservationSearch {
	...
	public String getObservation(
		...
		jobSearchDetails.setSearchOn(SearchOn.OBSERVATIONID.getSearchOn());
		jobSearchDetails.setSearchType("ObservationId");
		jobSearchDetails.setSearchParameters(jobSearch.getSearchParameters()
				.get(1).getValue());
		jobSearchDetails.setScriptType(SearchScript.OBSERVATIONID
				.getSearchScript());
		jobSearchDetails.setScriptParameters(jobSearchService
				.getScriptParameters(jobSearch));
	...
	public String getPatient(@RequestParam("batchFile") MultipartFile batchFile, Model model) {
		...
		try {
			...
			JobSearchDetails jobSearchDetails = new JobSearchDetails();
			jobSearchDetails.setSearchOn(SearchOn.PATIENTIDUSERPREFSBATCH.getSearchOn());
			jobSearchDetails.setSearchType("PatientIdFromFile");
	...
	@RequestMapping(value = "/HiveQLBatch", method = RequestMethod.POST)
	public String getObservationDetailsHive(
			@Valid @ModelAttribute("observationSearchForm") ObservationSearchForm observationForm,
			BindingResult result, Model model) {
		...
		// Invoke service which inserts a record in the table, calls the hadoop job
		assert (jobSearchService != null);
		...
		List<JobSearchParameter> searchParameters = new ArrayList<JobSearchParameter>(1);
		JobSearchParameter jobSearchParameter = new JobSearchParameter(
				JobSearchConstants.PATIENTID.getSearchKey(), observationForm.getPatientId(), 1);

		searchParameters.add(jobSearchParameter);

		jobSearchParameter = new JobSearchParameter(
				JobSearchConstants.OBSERVATIONID.getSearchKey(), observationForm.getObservationId(), 1);
		searchParameters.add(jobSearchParameter);
		...
		JobSearchDetails jobSearchDetails = new JobSearchDetails();
		jobSearchDetails.setSearchOn(SearchOn.OBSERVATIONIDHIVE.getSearchOn());
		jobSearchDetails.setSearchType("ObservationIdHive");
		jobSearchDetails.setSearchParameters(jobSearch.getSearchParameters().get(1).getValue());
		jobSearchDetails.setScriptType(SearchScript.OBSERVATIONIDHIVE.getSearchScript());
	...
	@RequestMapping(value = "/HiveQLBatch", method = RequestMethod.GET)
	public String observationSearchHive(ModelMap model) {
	...
	@RequestMapping(value = "/FileUploadHiveQL", method = RequestMethod.POST)
	public String getHiveBatchSearch(@RequestParam("batchFile") MultipartFile batchFile, Model model) {
		// Invoke service which inserts a record in the table, calls the hadoop
		// job
		assert (jobSearchService != null);
		User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		...
		try {
			...
			JobSearchDetails jobSearchDetails = new JobSearchDetails();
			jobSearchDetails.setSearchOn(SearchOn.PATIENTIDUSERPREFSBATCHHIVE.getSearchOn());
			jobSearchDetails.setSearchType("PatientIdFromFileHive");
			jobSearchDetails.setScriptType(SearchScript.PATIENTIDUSERPREFSHIVE.getSearchScript());
			jobSearchDetails.setScriptParameters(jobSearchService.getPatientSearchWithUserPrefsParameters(jobSearch));
	...
	@RequestMapping(value = "/FileUploadHiveQL", method = RequestMethod.GET)
	public String hiveBatchSearch(ModelMap model) {
		// Allow the user to search for a patient
		return "PatientSearchBatch";
	}
}
