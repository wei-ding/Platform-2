...
gulp.task('html', ['inject', 'partials'], function () {
  ...
  var htmlFilter = $.filter(['*.html', '!/client/app/elements/examples/*.html']);
...
gulp.task('translations', function () {
  return gulp.src('client/**/il8n/*.json')
    .pipe(gulp.dest(paths.dist + '/'))
    .pipe($.size());
});

gulp.task('data', function () {
  return gulp.src('client/**/data/*.json')
    .pipe(gulp.dest(paths.dist + '/'))
    .pipe($.size());
});

gulp.task('examplejs', function () {
  return gulp.src('client/**/examples/*.{js,scss}')
    .pipe(gulp.dest(paths.dist + '/'))
    .pipe($.size());
});
