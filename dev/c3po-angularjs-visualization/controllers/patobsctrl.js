appv.controller('patobsctrl', ['$scope',function($scope){
}]);
appv.directive('myNode', ['$compile','$http','$routeParams','$location','url', function ($compile,$http,$routeParams,$location,url) {
	return {
		restrict: 'A',
		...
		$http.get(url+'ang/Visualization/'+$routeParams.val+'/json/'+$routeParams.id).success(function(data){
			patient_data=data;
			...
			for (var i=0; i<attributes.length; i++){       
				...
				for (var j=0;j<filtered_data[i].length;j++) {
					...
					parentKey = filtered_data[i][j].time_series;
					draw_timeseries1(filtered_data[i][j],parentKey,i,1)
				}
			}
			return data;
		}).error(function(data){
