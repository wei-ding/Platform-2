...
var fluctuationChart = dw.barChart("#fluctuation-chart");
var quarterChart = dw.pieChart("#quarter-chart");
var dayOfWeekChart = dw.rowChart("#day-of-week-chart");
var moveChart = dw.lineChart("#monthly-move-chart");
var visitChart = dw.barChart("#monthly-visit-chart");
var yearlyBubbleChart = dw.bubbleChart("#yearly-bubble-chart");
...
//```
d3.csv("data/visit.csv", function (data) {
    ...
    data.forEach(function (d) {
        ...
        d.lastday= +d.lastday; // coerce to number
        d.nextday= +d.nextday;
    });

    //### Create Crossfilter Dimensions and Groups
    //See the [crossfilter API](https://github.com/square/crossfilter/wiki/API-Reference) for reference.
    var visit = crossfilter(data);
    var all = visit.groupAll();

    // dimension by year
    var yearlyDimension = visit.dimension(function (d) {
    ...
    var yearlyPerformanceGroup = yearlyDimension.group().reduce(
        ...
        function (p, v) {
            ++p.count;
            p.absGain += v.nextday- v.lastday;
            p.fluctuation += Math.abs(v.nextday- v.lastday);
            p.sumIndex += (v.lastday+ v.nextday) / 2;
        ...
        function (p, v) {
            ...
            p.absGain -= v.nextday- v.lastday;
            p.fluctuation -= Math.abs(v.nextday-v.lastday);
            p.sumIndex -= (v.nextday+ v.lastday) / 2;
            p.avgIndex = p.sumIndex / p.count;
            p.percentageGain = (p.absGain / p.avgIndex) * 100;
            p.fluctuationPercentage = (p.fluctuation / p.avgIndex) * 100;
            return p;
    ...
    var dateDimension = visit.dimension(function (d) {
        return d.dd;
    });

    // dimension by month
    var moveMonths = visit.dimension(function (d) {
        return d.month;
    });
    // group by total movement within month
    var monthlyMoveGroup = moveMonths.group().reduceSum(function (d) {
        return Math.abs(d.nextday- d.lastday);
    });
    // group by total volume within move, and scale down result
    var volumeByMonthGroup = moveMonths.group().reduceSum(function (d) {
        return d.visit / 500000;
    });
    var indexAvgByMonthGroup = moveMonths.group().reduce(
        function (p, v) {
            ++p.days;
            p.total += (v.lastday+ v.nextday) / 2;
    ...
    var gainOrLoss = visit.dimension(function (d) {
        return d.lastday > d.nextday? "M" : "F";
    });
    // produce counts records in the dimension
    var gainOrLossGroup = gainOrLoss.group();

    // determine a histogram of percent changes
    var fluctuation = visit.dimension(function (d) {
        return Math.round((d.nextday- d.lastday) / d.lastday* 100);
    });
    var fluctuationGroup = fluctuation.group();

    // summerize volume by quarter
    var quarter = visit.dimension(function (d) {
    ...
    // counts per weekday
    var dayOfWeek = visit.dimension(function (d) {
        var day = d.dd.getDay();
    ...
    /* dw.bubbleChart("#yearly-bubble-chart", "chartGroup") */
    yearlyBubbleChart
        ...
        .xAxisLabel('Index Visit') // (optional) render an axis label below the x axis
        .yAxisLabel('Index Visit%') // (optional) render a vertical axis lable left of the y axis
        //#### Labels and  Titles
        ...
        .title(function (p) {
            return [p.key,
                   "Index Visit: " + numberFormat(p.value.absGain),
                   "Index Visit in Percentage: " + numberFormat(p.value.percentageGain) + "%",
    ...
    // to a specific group then any interaction with such chart will only trigger redraw
    // on other charts within the same chart group.
    /* dw.barChart("#volume-month-chart") */
    ...
    moveChart
        ...
        .group(indexAvgByMonthGroup, "Monthly Visit Patients")
    ...
    */
    dw.dataTable(".dw-data-table")
        ...
        .columns([
            ...
            /*
            function (d) {
                return numberFormat(d.lastday);
            },
            function (d) {
                return numberFormat(d.nextday);
            },
            function (d) {
                return numberFormat(d.nextday - d.lastday);
            },
            */
            function (d) {
                return d.visit;
        ...
        .renderlet(function (table) {
            table.selectAll(".dw-table-group").classed("info", true);
        });
    ...
    //to a specific group then any interaction with such chart will only trigger redraw
    //on other charts within the same chart group.
    dw.geoChoroplethChart("#us-chart")
        .width(990) // (optional) define chart width, :default = 200
        ...
        // to a specific group then any interaction with such chart will only trigger redraw
        // on other charts within the same chart group.
        dw.bubbleOverlay("#bubble-overlay")
