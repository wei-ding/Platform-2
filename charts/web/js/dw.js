...
The dw.js filters are functions which are passed into crossfilter to chose which records will be
accumulated to produce values for the charts.  In the crossfilter model, any filters applied on one
dimension will affect all the other dimensions but not that one.  dw always applies a filter
...
/**
#### dw.filters.TwoDimensionalFilter(array)
...
**/
dw.filters.TwoDimensionalFilter = function(array) {
...
/**
#### dw.filters.RangedTwoDimensionalFilter(array)
 ...
 x2`.
 **/
dw.filters.RangedTwoDimensionalFilter = function(array){
    if (array === null) { return null; }
...
/**
## Base Mixin
Base Mixin is an abstract functional object representing a basic dw chart object
for all chart and widget implementations. Methods from the Base Mixin are inherited
and available on all chart implementation in the dw library.
**/
dw.baseMixin = function (_chart) {
    _chart.__dw_flag__ = dw.utils.uniqueId();
    ...
    var _keyAccessor = dw.pluck('key');
    var _valueAccessor = dw.pluck('value');
    var _label = dw.pluck('key');

    var _ordering = dw.pluck('key');
    var _orderSort;
    ...
    var _filterPrinter = dw.printers.filters;

    var _renderlets = [];
    var _mandatoryAttributes = ['dimension', 'group'];

    var _chartGroup = dw.constants.DEFAULT_CHART_GROUP;
    ...
    **/
    ...
    Set or get the dimension attribute of a chart. In dw a dimension can be any valid [crossfilter
    ...
    **/
    ...
    Set or get the group attribute of a chart. In dw a group is a [crossfilter
    ...
    **/
    _chart.anchor = function (a, chartGroup) {
        ...
        if (dw.instanceOfChart(a)) {
            _anchor = a.anchor();
            _root = a.root();
        } else {
            _anchor = a;
            _root = d3.select(_anchor);
            _root.classed(dw.constants.CHART_CLASS, true);
            dw.registerChart(_chart, chartGroup);
        }
    ...
    **/
    ...
    dw internally. Resetting the root element on a chart outside of dw internals may have
    ...
    **/
    ...
    however this is usually handled by dw internally. Resetting the svg element on a chart outside
    of dw internals may have unexpected consequences.
    ...
    **/
    ...
    friendly text for filter value(s) associated with the chart instance. By default dw charts use a
    default filter printer `dw.printers.filter` that provides simple printing support for both
    ...
    **/
    ...
    Turn on/off optional control elements within the root element. dw currently supports the
    ...
    **/
    ...
    function checkForMandatoryAttributes(a) {
        if (!_chart[a] || !_chart[a]())
            throw new dw.errors.InvalidStateException("Mandatory attribute chart." + a +
    ...
    **/
    ...
    chart. Most chart interaction in dw will automatically trigger this method through internal
    events (in particular [dw.redrawAll](#dwredrawallchartgroup)); therefore, you only need to
    manually invoke this function if data is manipulated outside of dw's control (for example if
    ...
    **/
    ...
    _chart.redrawGroup = function () {
        dw.redrawAll(_chart.chartGroup());
    };

    _chart.renderGroup = function () {
        dw.renderAll(_chart.chartGroup());
    };
    ...
    **/
    ...
    _chart.highlightSelected = function (e) {
        d3.select(e).classed(dw.constants.SELECTED_CLASS, true);
        d3.select(e).classed(dw.constants.DESELECTED_CLASS, false);
    };

    _chart.fadeDeselected = function (e) {
        d3.select(e).classed(dw.constants.SELECTED_CLASS, false);
        d3.select(e).classed(dw.constants.DESELECTED_CLASS, true);
    };
    ...
    **/
    ...
    use the dw API or use raw d3 to achieve pretty much any effect.
    ```js
    // renderlet function
    chart.renderlet(function(chart){
        // mix of dw API and d3 manipulation
    ...
    **/
    ...
    /**
    #### .expireCache()
    Expire the internal chart cache. dw charts cache some data internally on a per chart basis to
    ...
    **/
    ...
    /**
    #### .legend([dw.legend])
    Attach a dw.legend widget to this chart. The legend widget will automatically draw legend labels
    based on the color setting and names associated with each group.

    ```js
    chart.legend(dw.legend().x(400).y(10).itemHeight(13).gap(5))
    ```
    ...
    **/
    _chart.options = function(opts) {
        for (var o in opts) {
            if (typeof(_chart[o]) === 'function') {
                ...
                dw.logger.debug("Not a valid option setter name: " + o);
            }
        }
        return _chart;
    };
    ...
    /**
    ## Listeners
    All dw chart instance supports the following listeners.
...
**/
dw.marginMixin = function (_chart) {
...
**/
dw.coordinateGridMixin = function (_chart) {
    ...
    _chart = dw.colorMixin(dw.marginMixin(dw.baseMixin(_chart)));
    ...
    **/
    ...
    overlay custom svg drawing programatically. **Caution**: 
    ...
    **/
    ...
    useful when x axis customization is required. The x axis in dw.js is an instance of a [d3
    axis object](https://github.com/mbostock/d3/wiki/SVG-Axes#wiki-axis); therefore it supports any
    valid d3 axis manipulation. **Caution**: The x axis is usually generated internally by dw;
    ...
    **/
    ...
    /**
    #### isOrdinal()
    Returns true if the chart is using ordinal xUnits ([dw.units.ordinal](#dwunitsordinal)), or false
    otherwise. Most charts behave differently with ordinal data and use the result of this method to
    trigger the appropriate logic.
    **/
    _chart.isOrdinal = function () {
        return _chart.xUnits() === dw.units.ordinal;
    };
    ...
    _chart.renderXAxis = function (g) {
        ...
        dw.transition(axisXG, _chart.transitionDuration())
            .call(_xAxis);
    };
    ...
    function renderVerticalGridLines(g) {
        ...
        if (_renderVerticalGridLine) {
            ...
            var linesGEnter = lines.enter()
                ...
                .attr("y2", 0)
                .attr("opacity", 0);
            dw.transition(linesGEnter, _chart.transitionDuration())
                .attr("opacity", 1);

            // update
            dw.transition(lines, _chart.transitionDuration())
                .attr("x1", function (d) {
                    return _x(d);
                })
                .attr("y1", _chart._xAxisY() - _chart.margins().top)
    ...
    **/
    ...
    _chart.renderYAxisAt = function (axisClass, axis, position){
        ...
        dw.transition(axisYG, _chart.transitionDuration()).call(axis);
    };
    ...
    **/
    ...
    when y axis customization is required. The y axis in dw.js is simply an instance of a [d3 axis
    object](https://github.com/mbostock/d3/wiki/SVG-Axes#wiki-_axis); therefore it supports any
    valid d3 axis manipulation. **Caution**: The y axis is usually generated internally by dw;
    resetting it may cause unexpected results.
    ```js
    ...
    **/
    _chart.xAxisMin = function () {
        ...
        return dw.utils.subtract(min, _xAxisPadding);
    };
    ...
    **/
    _chart.xAxisMax = function () {
        ...
        return dw.utils.add(max, _xAxisPadding);
    };
    ...
    **/
    _chart.yAxisMin = function () {
        ...
        return dw.utils.subtract(min, _yAxisPadding);
    };
    ...
    **/
    _chart.yAxisMax = function () {
        ...
        return dw.utils.add(max, _yAxisPadding);
    };
    ...
    **/
    ...
    dw.override(_chart, "filter", function (_) {
    ...
    _chart._brushing = function() {
        ...
        if (_chart.brushIsEmpty(extent)) {
            dw.events.trigger(function () {
                _chart.filter(null);
                _chart.redrawGroup();
            }, dw.constants.EVENT_DELAY);
        } else {
            var rangedFilter = dw.filters.RangedFilter(extent[0], extent[1]);
    ...
    **/
    ...
    function generateClipPath() {
        var defs = dw.utils.appendOrSelect(_parent, "defs");

        var chartBodyClip = dw.utils.appendOrSelect(defs, "clipPath").attr("id", getClipPathId());

        var padding = _clipPadding * 2;

        dw.utils.appendOrSelect(chartBodyClip, "rect")
    ...
    function zoomHandler() {
        ...
        var domFilter = dw.filters.RangedFilter(domain[0], domain[1]);
...
dw.stackMixin = function (_chart) {
    ...
    **/
    ...
    dw.override(_chart,'group', function (g,n,f) {
    ...
    **/
    ...
    function findLayerByName(n) {
        var i = _stack.map(dw.pluck('name')).indexOf(n);
        return _stack[i];
    }
    ...
    **/
    ...
    _chart.yAxisMin = function () {
        ...
        return dw.utils.subtract(min, _chart.yAxisPadding());
    ...
    _chart.yAxisMax = function () {
        ...
        return dw.utils.add(max, _chart.yAxisPadding());
    };
    ...
    _chart.xAxisMin = function () {
        var min = d3.min(flattenStack(), dw.pluck('x'));
        return dw.utils.subtract(min, _chart.xAxisPadding());
    };

    _chart.xAxisMax = function () {
        var max = d3.max(flattenStack(), dw.pluck('x'));
        return dw.utils.add(max, _chart.xAxisPadding());
    };
    ...
    ```
    **/
    dw.override(_chart, "title", function (stackName, titleAccessor) {
    ...
    _chart._ordinalXDomain = function () {
        return flattenStack().map(dw.pluck('x'));
    };
...
dw.capMixin = function (_chart) {
    ...
    **/
    ...
    dw.override(_chart, "onClick", function (d) {
        if (d.others)
...
**/
dw.bubbleMixin = function (_chart) {
    ...
    _chart._doRenderLabel = function (bubbleGEnter) {
        if (_chart.renderLabel()) {
            ...
            label
                .attr("opacity", 0)
                .text(labelFunction);
            dw.transition(label, _chart.transitionDuration())
    ...
    _chart.doUpdateLabels = function (bubbleGEnter) {
        if (_chart.renderLabel()) {
            var labels = bubbleGEnter.selectAll("text")
                .text(labelFunction);
            dw.transition(labels, _chart.transitionDuration())
                .attr("opacity", labelOpacity);
        }
    };
...
var chart1 = dw.pieChart("#chart-container1");
// create a pie chart under #chart-container2 element using chart group A
var chart2 = dw.pieChart("#chart-container2", "chartGroupA");
```

**/
dw.pieChart = function (parent, chartGroup) {
    ...
    var _chart = dw.capMixin(dw.colorMixin(dw.baseMixin({})));
    ...
    **/
    ...
    function createSlicePath(slicesEnter, arc) {
        ...
        dw.transition(slicePath, _chart.transitionDuration(), function (s) {
    ...
    function positionLabels(labelsEnter, arc) {
        dw.transition(labelsEnter, _chart.transitionDuration())
    ...
    function updateSlicePaths(pieData, arc) {
        ...
        dw.transition(slicePaths, _chart.transitionDuration(),
            function (s) {
                s.attrTween("d", tweenPie);
            }).attr("fill", fill);
    }
...
var chart1 = dw.barChart("#chart-container1");
// create a bar chart under #chart-container2 element using chart group A
var chart2 = dw.barChart("#chart-container2", "chartGroupA");
// create a sub-chart under a composite parent chart
var chart3 = dw.barChart(compositeChart);
```

**/
dw.barChart = function (parent, chartGroup) {
    var MIN_BAR_WIDTH = 1;
    var DEFAULT_GAP_BETWEEN_BARS = 2;

    var _chart = dw.stackMixin(dw.coordinateGridMixin({}));
    ...
    dw.override(_chart, 'rescale', function () {
        _chart._rescale();
        _barWidth = undefined;
    });

    dw.override(_chart, 'render', function () {
        if (_chart.round() && _centerBar && !_alwaysUseRounding) {
            dw.logger.warn("By default, brush rounding is disabled if bars are centered. " +
                         "See dw.js bar chart API documentation for details.");
        }
    ...
    function barHeight(d) {
        return dw.utils.safeNumber(Math.abs(_chart.y()(d.y + d.y0) - _chart.y()(d.y0)));
    }

    function renderBars(layer, layerIndex, d) {
        var bars = layer.selectAll("rect.bar")
            .data(d.values, dw.pluck('x'));

        var enter = bars.enter()
            .append("rect")
            .attr("class", "bar")
            .attr("fill", dw.pluck('data',_chart.getColor))
            .attr("height", 0);

        if (_chart.renderTitle())
            enter.append("title").text(dw.pluck('data',_chart.title(d.name)));

        if (_chart.isOrdinal())
            bars.on("click", onClick);

        dw.transition(bars, _chart.transitionDuration())
            .attr("x", function (d) {
                var x = _chart.x()(d.x);
                if (_centerBar) x -= _barWidth / 2;
                if (_chart.isOrdinal() && _gap!==undefined) x += _gap/2;
                return dw.utils.safeNumber(x);
            })
            .attr("y", function (d) {
            ...
            .attr("fill", dw.pluck('data',_chart.getColor))
            .select("title").text(dw.pluck('data',_chart.title(d.name)));

        dw.transition(bars.exit(), _chart.transitionDuration())
            .attr("height", 0)
            .remove();
    }
    ...
    _chart.fadeDeselectedArea = function () {
        ...
        if (_chart.isOrdinal()) {
            if (_chart.hasFilter()) {
                bars.classed(dw.constants.SELECTED_CLASS, function (d) {
                    return _chart.hasFilter(d.x);
                });
                bars.classed(dw.constants.DESELECTED_CLASS, function (d) {
                    return !_chart.hasFilter(d.x);
                });
            } else {
                bars.classed(dw.constants.SELECTED_CLASS, false);
                bars.classed(dw.constants.DESELECTED_CLASS, false);
            }
        } else {
            if (!_chart.brushIsEmpty(extent)) {
                var start = extent[0];
    ...
    **/
    ...
    dw.override(_chart, "xAxisMax", function() {
...
#### dw.lineChart(parent[, chartGroup])
...
var chart1 = dw.lineChart("#chart-container1");
// create a line chart under #chart-container2 element using chart group A
var chart2 = dw.lineChart("#chart-container2", "chartGroupA");
// create a sub-chart under a composite parent chart
var chart3 = dw.lineChart(compositeChart);
```

**/
dw.lineChart = function (parent, chartGroup) {
    var DEFAULT_DOT_RADIUS = 5;
    var TOOLTIP_G_CLASS = "dw-tooltip";
    ...
    /**
     ...
     https://github.com/dw-js/dw.js/issues/615#issuecomment-49089248
     **/
    ...
    **/
    ...
    function drawLine(layersEnter, layers) {
        ...
        dw.transition(layers.select("path.line"), _chart.transitionDuration())
            //.ease("linear")
            .attr("stroke", colors)
            .attr("d", function (d) {
    ...
    function drawArea(layersEnter, layers) {
        if (_renderArea) {
            ...
            dw.transition(layers.select("path.area"), _chart.transitionDuration())
                //.ease("linear")
                .attr("fill", colors)
                .attr("d", function (d) {
                    return safeD(area(d.values));
        ...
        if (!_chart.brushOn()) {
            ...
            layers.each(function (d, layerIndex) {
                ...
                dots.enter()
                    ...
                    .append("title").text(dw.pluck('data', _chart.title(d.name)));

                dots.attr("cx", function (d) {
                        return dw.utils.safeNumber(_chart.x()(d.x));
                    })
                    .attr("cy", function (d) {
                        return dw.utils.safeNumber(_chart.y()(d.y + d.y0));
                    })
                    .attr("fill", _chart.getColor)
                    .select("title").text(dw.pluck('data', _chart.title(d.name)));
    ...
    **/
    ...
    dw.override(_chart,'legendables', function() {
...
#### dw.dataCount(parent[, chartGroup])
...
**/
dw.dataCount = function(parent, chartGroup) {
    var SPAN_CLASS = 'data-count-display';
    var _formatNumber = d3.format(",d");
    var _chart = dw.baseMixin({});
    var _html = {some:"",all:""};
...
#### dw.dataTable(parent[, chartGroup])
...
**/
dw.dataTable = function(parent, chartGroup) {
    var LABEL_CSS_CLASS = "dw-table-label";
    var ROW_CSS_CLASS = "dw-table-row";
    var COLUMN_CSS_CLASS = "dw-table-column";
    var GROUP_CSS_CLASS = "dw-table-group";
 ...
 **/
dw.dataGrid = function(parent, chartGroup) {
    var LABEL_CSS_CLASS = "dw-grid-label";
    var ITEM_CSS_CLASS = "dw-grid-item";
    var GROUP_CSS_CLASS = "dw-grid-group";
    var GRID_CSS_CLASS = "dw-grid-top";
...
#### dw.bubbleChart(parent[, chartGroup])
...
var bubbleChart1 = dw.bubbleChart("#chart-container1");
// create a bubble chart under #chart-container2 element using chart group A
var bubbleChart2 = dw.bubbleChart("#chart-container2", "chartGroupA");
```

**/
dw.bubbleChart = function(parent, chartGroup) {
    var _chart = dw.bubbleMixin(dw.coordinateGridMixin({}));
    ...
    **/
    ...
    function renderNodes(bubbleG) {
        ...
        bubbleGEnter
            ...
            .attr("fill", _chart.getColor)
            .attr("r", 0);
        dw.transition(bubbleG, _chart.transitionDuration())
...
#### dw.compositeChart(parent[, chartGroup])
...
var compositeChart1 = dw.compositeChart("#chart-container1");
// create a composite chart under #chart-container2 element using chart group A
var compositeChart2 = dw.compositeChart("#chart-container2", "chartGroupA");
```

**/
dw.compositeChart = function (parent, chartGroup) {

    var SUB_CHART_CLASS = "sub";
    var DEFAULT_RIGHT_Y_AXIS_LABEL_PADDING = 12;

    var _chart = dw.coordinateGridMixin({});
    var _children = [];
    ...
    dw.override(_chart, "_generateG", function () {
    ...
    **/
    ...
    function yAxisMax() {
        return dw.utils.add(d3.max(getYAxisMax(leftYAxisChildren())), _chart.yAxisPadding());
    }

    function rightYAxisMax() {
        return dw.utils.add(d3.max(getYAxisMax(rightYAxisChildren())), _chart.yAxisPadding());
    }

    function getAllXAxisMinFromChildwharts() {
        return _children.map(function(c) {
            return c.xAxisMin();
        });
    }

    dw.override(_chart, 'xAxisMin',function () {
        return dw.utils.subtract(d3.min(getAllXAxisMinFromChildwharts()), _chart.xAxisPadding());
    });

    function getAllXAxisMaxFromChildwharts() {
        return _children.map(function(c) {
            return c.xAxisMax();
        });
    }
    ...
    dw.override(_chart, 'xAxisMax',function () {
        return dw.utils.add(d3.max(getAllXAxisMaxFromChildwharts()), _chart.xAxisPadding());
    });
    ...
    axis customization is required. The y axis in dw.js is an instance of a [d3 axis
    object](https://github.com/mbostock/d3/wiki/SVG-Axes#wiki-_axis) therefore it supports any valid
    d3 axis manipulation. **Caution**: The y axis is usually generated internally by dw;
    resetting it may cause unexpected results.
    ```js
    // customize y axis tick format
...
/**
 ...
 #### dw.seriesChart(parent[, chartGroup])
...
* chartGroup : string (optional) - name of the chart group this chart instance should be placed in.
 ...
 var seriesChart1 = dw.seriesChart("#chart-container1");
 // create a series chart under #chart-container2 element using chart group A
 var seriesChart2 = dw.seriesChart("#chart-container2", "chartGroupA");
 ```

 **/
dw.seriesChart = function (parent, chartGroup) {
    var _chart = dw.compositeChart(parent, chartGroup);

    var _charts = {};
    var _chartFunction = dw.lineChart;
    var _seriesAccessor;
    ...
    /**
     #### .chart([function])
     Get or set the chart function, which generates the child charts.  Default: dw.lineChart

     ```
     // put interpolation on the line charts used for the series
     chart.chart(function(c) { return dw.lineChart(c).interpolate('basis'); })
     // do a scatter series chart
     chart.chart(dw.scatterPlot)
     ```
...
#### dw.geoChoroplethChart(parent[, chartGroup])
...
var chart1 = dw.geoChoroplethChart("#us-chart");
// create a choropleth chart under "#us-chart2" element using chart group A
var chart2 = dw.compositeChart("#us-chart2", "chartGroupA");
```

**/
dw.geoChoroplethChart = function (parent, chartGroup) {
    var _chart = dw.colorMixin(dw.baseMixin({}));
    ...
    function renderRegionG(layerIndex) {
        var regionG = _chart.svg()
            ...
            .attr("class", function (d) {
                ...
                var regionClass = dw.utils.nameToId(geoJson(layerIndex).keyAccessor(d));
    ...
    function renderPaths(regionG, layerIndex, data) {
        ...
        dw.transition(paths, _chart.transitionDuration()).attr("fill", function (d, i) {
...
#### dw.bubbleOverlay(parent[, chartGroup])
...
var bubbleChart1 = dw.bubbleOverlayChart("#chart-container1").svg(d3.select("#chart-container1 svg"));
// create a bubble overlay chart on top of the "#chart-container2 svg" element using chart group A
var bubbleChart2 = dw.compositeChart("#chart-container2", "chartGroupA").svg(d3.select("#chart-container2 svg"));
```

#### .svg(imageElement) - **mandatory**
Set the underlying svg image element. Unlike other dw charts this chart will not generate a svg
...
**/
dw.bubbleOverlay = function(root, chartGroup) {
    var BUBBLE_OVERLAY_CLASS = "bubble-overlay";
    var BUBBLE_NODE_CLASS = "node";
    var BUBBLE_CLASS = "bubble";

    var _chart = dw.bubbleMixin(dw.baseMixin({}));
    var _g;
    var _points = [];
    ...
    **/
    ...
    function initializeBubbles() {
        ...
        _points.forEach(function(point) {
            ...
            dw.transition(circle, _chart.transitionDuration())
                .attr("r", function(d) {
                    return _chart.bubbleR(d);
                });
    ...
    function getNodeG(point, data) {
        var bubbleNodeClass = BUBBLE_NODE_CLASS + " " + dw.utils.nameToId(point.name);

        var nodeG = _g.select("g." + dw.utils.nameToId(point.name));
    ...
    _chart.debug = function(flag) {
        if(flag){
            var debugG = _chart.select("g." + dw.constants.DEBUG_GROUP_CLASS);

            if(debugG.empty())
                debugG = _chart.svg()
                    .append("g")
                    .attr("class", dw.constants.DEBUG_GROUP_CLASS);
...
var chart1 = dw.rowChart("#chart-container1");
// create a row chart under #chart-container2 element using chart group A
var chart2 = dw.rowChart("#chart-container2", "chartGroupA");
```

**/
dw.rowChart = function (parent, chartGroup) {
    ...
    var _chart = dw.capMixin(dw.marginMixin(dw.colorMixin(dw.baseMixin({}))));
    ...
    function drawAxis() {
        ...
        dw.transition(axisG, _chart.transitionDuration())
            .call(_xAxis);
    }
    ...
    function updateElements(rows) {
        ...
        dw.transition(rect, _chart.transitionDuration())
    ...
    function updateLabels(rows) {
        if (_chart.renderLabel()) {
            ...
            dw.transition(lab, _chart.transitionDuration())
                .attr("transform", translateX);
        }
        if (_chart.renderTitleLabel()) {
            ...
            dw.transition(titlelab, _chart.transitionDuration())
                .attr("transform", translateX);
        }
    }
...
/**
## Legend
Legend is a attachable widget that can be added to other dw charts to render horizontal legend
labels.
...
dw.legend = function () {
    ...
    _legend.render = function () {
        _parent.svg().select("g.dw-legend").remove();
        _g = _parent.svg().append("g")
            .attr("class", "dw-legend")
            .attr("transform", "translate(" + _x + "," + _y + ")");
        var legendables = _parent.legendables();

        var itemEnter = _g.selectAll('g.dw-legend-item')
            .data(legendables)
            .enter()
            .append("g")
            .attr("class", "dw-legend-item")
        ...
        _g.selectAll('g.dw-legend-item')
            .classed("fadeout", function(d) {
                return d.chart.isLegendableHidden(d);
            });

        if (legendables.some(dw.pluck('dashstyle'))) {
            itemEnter
                .append("line")
                .attr("x1", 0)
                ...
                .attr("stroke-width", 2)
                .attr("stroke-dasharray", dw.pluck('dashstyle'))
                .attr("stroke", dw.pluck('color'));
        } else {
            itemEnter
                .append("rect")
...
#### dw.scatterPlot(parent[, chartGroup])
...
var chart1 = dw.scatterPlot("#chart-container1");
// create a scatter plot under #chart-container2 element using chart group A
var chart2 = dw.scatterPlot("#chart-container2", "chartGroupA");
// create a sub-chart under a composite parent chart
var chart3 = dw.scatterPlot(compositeChart);
```

 **/
dw.scatterPlot = function (parent, chartGroup) {
    var _chart = dw.coordinateGridMixin({});
    ...
    dw.override(_chart, "_filter", function(filter) {
        if (!arguments.length) return _chart.__filter();

        return _chart.__filter(dw.filters.RangedTwoDimensionalFilter(filter));
    });
    ...
    _chart.plotData = function () {
        ...
        dw.transition(symbols, _chart.transitionDuration())
            .attr("opacity", function(d) { return d.value ? 1 : 0; })
            .attr("fill", _chart.getColor)
            .attr("transform", _locator)
            .attr("d", _symbol);

        dw.transition(symbols.exit(), _chart.transitionDuration())
    ...
    **/
    ...
    function resizeSymbolsWhere(condition, size) {
        ...
        dw.transition(symbols, _chart.transitionDuration()).attr("d", _symbol);
    ...
    _chart._brushing = function () {
        ...
        if (_chart.brushIsEmpty(extent)) {
            ...
            var ranged2DFilter = dw.filters.RangedTwoDimensionalFilter(extent);
            dw.events.trigger(function () {
                _chart.filter(null);
...
#### dw.numberDisplay(parent[, chartGroup])
...
var display1 = dw.numberDisplay("#chart-container1");
```

**/
dw.numberDisplay = function (parent, chartGroup) {
...
* chartGroup : string (optional) - name of the chart group this chart instance should be placed in.
 ...
 var heatMap1 = dw.heatMap("#chart-container1");
 // create a heat map under #chart-container2 element using chart group A
 var heatMap2 = dw.heatMap("#chart-container2", "chartGroupA");
 ```

 **/
dw.heatMap = function (parent, chartGroup) {
    ...
    var _chart = dw.colorMixin(dw.marginMixin(dw.baseMixin({})));
    _chart._mandatoryAttributes(['group']);
    ...
    function filterAxis(axis, value) {
        ...
        var unfilteredwellsOnAxis = cellsOnAxis.filter( function (d) {
            return !_chart.hasFilter(d.key);
        });
        dw.events.trigger(function() {
            if(unfilteredwellsOnAxis.empty()) {
                cellsOnAxis.each( function (d) {
                    _chart.filter(d.key);
                });
            } else {
                unfilteredwellsOnAxis.each( function (d) {
                    _chart.filter(d.key);
                });
            }
            _chart.redrawGroup();
    ...
    dw.override(_chart, "filter", function(filter) {
        if (!arguments.length) return _chart._filter();

        return _chart._filter(dw.filters.TwoDimensionalFilter(filter));
    });
    ...
    _chart._doRedraw = function () {
        ...
        dw.transition(boxes.selectAll("rect"), _chart.transitionDuration())
        ...
        gRows.selectAll('text').data(rows.domain())
            .enter().append("text")
              ...
              .text(function(d) { return d; });
        dw.transition(gRows.selectAll('text'), _chart.transitionDuration())
              .text(function(d) { return d; })
...
* chartGroup : string (optional) - name of the chart group this chart instance should be placed in.
 ...
 var boxPlot1 = dw.boxPlot("#chart-container1");
 // create a box plot under #chart-container2 element using chart group A
 var boxPlot2 = dw.boxPlot("#chart-container2", "chartGroupA");
 ```

 **/
dw.boxPlot = function (parent, chartGroup) {
    var _chart = dw.coordinateGridMixin({});
    ...
    _chart.xUnits(dw.units.ordinal);
    ...
    **/
    ...
    function updateBoxes(boxesG) {
        dw.transition(boxesG, _chart.transitionDuration())
            .attr("transform", boxTransform)
            .call(_box)
            .each(function() {
